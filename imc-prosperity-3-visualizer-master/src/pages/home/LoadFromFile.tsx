...
export function LoadFromFile(): ReactNode {
  ...
  const onReject = useCallback((rejections: FileRejection[]) => {
    ...
    for (const rejection of rejections) {
      const errorType = {
        'file-invalid-type': 'Invalid type, only log or JSON files are supported.',
  ...
  return (
    <HomeCard title="Load from file">
      <Text>
        Supports Prosperity server log files and original IMC result JSON files (the ones that include
        <code>activitiesLog</code> and <code>tradeHistory</code>).
      </Text>
      ...
      <Dropzone
        ...
        accept={{
          ...
          'application/octet-stream': ['.log', '.txt', '.json'],
        }}
      >
        <Dropzone.Idle>
          <DropzoneContent />
